...
package com.zui.animate.placeholder
...
/**
 * @param t 唯一不同添加了个泛型，如果为空的话展示占位符，反之不展示
 */
fun <T> Modifier.placeholder(
    t: T?
): Modifier {
    val color: Color = Color.LightGray
    val shape: Shape = RoundedCornerShape(4.dp)
    val highlight: PlaceholderHighlight = PlaceholderHighlight.shimmer(Color.Gray)
    val placeholderFadeTransitionSpec: @Composable Transition.Segment<Boolean>.() -> FiniteAnimationSpec<Float> =
        { spring() }
    val contentFadeTransitionSpec: @Composable Transition.Segment<Boolean>.() -> FiniteAnimationSpec<Float> =
        { spring() }
    return placeholder(
        t == null,
        color,
